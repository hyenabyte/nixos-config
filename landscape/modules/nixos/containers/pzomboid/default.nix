...
in {
  ...
  config = mkIf cfg.enable {
    ...
    # TODO
    systemd.services."restart-pzomboid" = {
      script = ''
        set -eu
        podman restart pzomboid
      '';
      serviceConfig = {
        Type = "oneshot";
        User = "root";
    ...
    systemd.timers."restart-pzomboid" = {
      timerConfig = {
    ...
    environment.etc = {
      "fail2ban/filter.d/pz-rcon-bruteforce.conf".text = ''
        [Definition]
        failregex = ^LOG[\ ]*: General[\ ,\d>]*RCON: connection closed Socket\[addr=\/<HOST>.*$
      '';
    };

    services.fail2ban.jails."pz-rcon-bruteforce" = ''
      enabled = true
      filter = pz-rcon-bruteforce
      logpath = ${cfg.configPath}/ZomboidConfig/server-console.txt
      maxretry = 5
      findtime = 600
    '';
